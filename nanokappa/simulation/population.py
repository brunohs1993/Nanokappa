...
class Population(Constants):
    ...
    def __init__(self, arguments, geometry, phonon):
        ...
        if self.particle_type == "pmps":
            ...
            self.particles_pmps = self.N_p / (
                phonon.number_of_active_modes * self.n_of_subvols
            )
            self.particle_density = self.N_p / geometry.volume
            ...
            try:
                data = np.loadtxt(
                    self.args.particles[0], delimiter=",", comments="#", dtype=float
                )
            except:
                raise Exception(
                    "Wrong particle data file. Change the keyword or check whether the file exists."
                )
        ...
        self.rough_facets_values = (
            geometry.rough_facets_values
        )  # their values of roughness and correlation length
        ...
        if self.args.reference_temp[0] != "local":
            ...
            self.reference_occupation = phonon.calculate_occupation(
                self.T_reference, phonon.omega
            )
        ...
        self.n_of_reservoirs = (self.bound_cond == "T").sum() + (
            self.bound_cond == "F"
        ).sum()
        ...
        self.view = Visualisation(
            self.args, geometry, phonon, self
        )  # initialising visualisation class
        self.plot_figures(
            geometry,
            phonon,
            property_plot=self.args.fig_plot,
            colormap=self.args.colormap[0],
        )
    ...
    def initialise_modes(self, phonon):
        ...
        # generate randomly
        modes = np.random.randint(
            low=0, high=phonon.number_of_active_modes, size=self.N_p
        )
    ...
    def enter_probability(self, geometry, phonon):
        ...
        bound_thickness = phonon.number_of_active_modes / (
            self.particle_density * geometry.facets_area[self.res_facet]
        )

        vel = np.transpose(
            phonon.group_vel, (0, 2, 1)
        )  # shape = (Q, 3, J) - Group velocities of each mode
        normals = -geometry.facets_normal[
            self.res_facet, :
        ]  # shape = (R, 3)    - unit normals of each facet with boundary conditions
        # OBS: normals are reversed to point inwards (in the direction of entering particles).
        group_vel_parallel = np.dot(
            normals, vel
        )  # shape = (R, Q, J) - dot product = projection of velocity over normal

        # Probability of a particle entering the domain
        enter_prob = (
            group_vel_parallel * self.dt / bound_thickness.reshape(-1, 1, 1)
        )  # shape = (R, Q, J)
        enter_prob = np.where(enter_prob < 0, 0, enter_prob)
        enter_prob /= enter_prob.sum(axis=(1, 2), keepdims=True)
        enter_prob = np.vstack(
            [enter_prob[i, :, :].ravel(order="F") for i in range(enter_prob.shape[0])]
        )  # Fortran order to match with phonon.unique_modes
    ...
    def initialise_all_particles(self, geometry, phonon):
        ...
        if sys.stdout.isatty():
            ...
            message = "{0:{3}d} of {1:{3}d} particles - {2:s}   0%".format(
                0, self.N_p, " " * bw, nchar
            )
        ...
        if self.args.particles[0] in ["total", "pv", "pmps"]:
            ...
            # assign temperatures
            self.temperatures, self.subvol_temperature = self.assign_temperatures(
            ...
            self.modes = np.copy(data[:, [0, 1]]).astype(
                int
            )  # need to add a check to see if the material is the same as the input
            self.positions = np.copy(
                data[:, [2, 3, 4]]
            )  # need to normalise and rescale positions to avoid conflicts
            ...
            while err > 1e-6:
                ...
                err = np.absolute(
                    (self.subvol_temperature - old_T) / self.subvol_temperature
                ).max()
        ...
        ) = self.timesteps_to_boundary(self.positions, self.group_vel, geometry)
    ...
    def initialise_reservoirs(self, geometry, phonon):
        ...
        mask_temp = (
            geometry.res_bound_cond == "T"
        )  # which RESERVOIRS (res1, res2, res3...) have imposed temperature, boolean
        mask_flux = (
            geometry.res_bound_cond == "F"
        )  # which RESERVOIRS (res1, res2, res3...) have imposed heat flux  , boolean
        ...
        self.res_facet_temperature = np.ones(
            self.n_of_reservoirs
        )  # initialise array with None, len = n_of_reservoirs
        self.res_facet_temperature[:] = None
        self.res_facet_temperature[
            mask_temp
        ] = facets_temp  # attribute imposed temperatures
        self.res_facet_temperature[
            mask_flux
        ] = facets_temp.mean()  # initialising with the mean of the imposed temperatures
    ...
    def select_reservoir_modes(
        self, collision_facets, omega_in, occupation_in, velocities_in, geometry, phonon
    ):
        ...
        for i in range(self.n_of_reservoirs):  # for each reservoir
            ...
            if n > 0:
                # picking new modes
                flat_i = np.random.choice(
                    phonon.number_of_modes,
                    size=n,
                    replace=True,
                    p=self.enter_prob[i, :],
                )
                ...
                energy = (
                    ...
                    * omega_in[indices]
                    * (
                        occupation_in[indices]
                        - phonon.calculate_occupation(
                            self.res_facet_temperature[i], omega_in[indices]
                        )
                    )
                )  # hbar*omega*dn
                heat_flux = (
                    energy.reshape(-1, 1)
                    * velocities_in[indices, :]
                    / (
                        velocities_in[indices, :]
                        * geometry.facets_normal[self.res_facet[i], :]
                    ).sum(axis=1, keepdims=True)
                )

                self.res_heat_flux[i, :] += heat_flux.sum(
                    axis=0
                )  # saving reservoir heat flux

                self.res_energy_balance[
                    i
                ] -= energy.sum()  # saving reservoir energy balance

                new_temperatures[indices] = self.res_facet_temperature[i]

        new_omega = phonon.omega[
            new_modes[:, 0], new_modes[:, 1]
        ]  # retrieve frequencies and occupations
    ...
    def assign_temperatures(self, positions, geometry):
        ...
        if (
            self.temp_interp_type in ["nearest", "linear"]
            and geometry.subvol_type == "slice"
        ):
            self.temp_interp = partial(
                interp1d, kind=self.temp_interp_type, fill_value="extrapolate"
            )
            ...
            if self.temp_interp_type == "linear":
                print(
                    "Linear T interpolation is currently valid for slice subvolumes only. Defaulting to RBF interpolation to avoid extrapolation problems."
                )
        ...
        if key == "custom":
            ...
            temperatures = subvol_temperatures[
                self.subvol_id
            ]  # .sum(axis = -1)/self.subvol_id.sum(axis = -1)

        else:
            bound_T = self.res_bound_values[
                self.res_bound_cond == "T"
            ]  # np.array([self.bound_values[i] for i in range(len(self.bound_values)) if self.bound_cond[i] == 'T'])
            ...
            if key == "linear":
                ...
                bound_positions = geometry.facet_centroid[
                    res_facet_index, :
                ]  # np.array([geometry.res_centroid[i, :] for i in range(len(self.bound_values)) if self.bound_cond[i] == 'T'])

                if len(bound_T) > 2:
                    d = geometry.subvol_center - np.expand_dims(
                        bound_positions, 1
                    )  # (R, SV, 3)
                    ...
                    K = (
                        (geometry.subvol_center - bound_positions[0, :]) * direction
                    ).sum(axis=1)
                    alphas = K / (direction**2).sum()

                    subvol_temperatures = bound_T[0] + alphas * (
                        bound_T[1] - bound_T[0]
                    )
                    temperatures = subvol_temperatures[self.subvol_id]
                ...
                subvol_temperatures = (
                    np.random.rand(self.n_of_subvols) * bound_T.ptp() + bound_T.min()
                )
                temperatures = subvol_temperatures[self.subvol_id]
            elif key == "hot":
        ...
        if geometry.subvol_type == "slice" and self.temp_interp_type in [
            "linear",
            "nearest",
        ]:
            self.temperature_interpolator = self.temp_interp(
                geometry.subvol_center[:, self.slice_axis], subvol_temperatures
            )
        elif geometry.subvol_type == "grid" and np.any(geometry.grid == 1):
            self.temperature_interpolator = self.temp_interp(
                geometry.subvol_center[:, geometry.grid != 1], subvol_temperatures
            )
        else:
            self.temperature_interpolator = self.temp_interp(
                geometry.subvol_center, subvol_temperatures
            )
    ...
    def get_collision_condition(self, collision_facets):
        ...
        collision_cond = np.empty(
            collision_facets.shape, dtype=str
        )  # initialise as an empty string array
    ...
    def get_subvol_id(self, positions, geometry, get_np=True, verbose=False):
        ...
        subvol_id = geometry.subvol_classifier.predict(
            positions
        )  # get subvol_id from the model

        if get_np:
            self.subvol_N_p = np.array(
                [(subvol_id == i).sum(dtype=int) for i in range(self.n_of_subvols)]
            )
    ...
    def refresh_temperatures(self, geometry, phonon):
        ...
        if geometry.subvol_type == "slice" and self.temp_interp_type in [
            ...
            self.temperature_interpolator = self.temp_interp(
                geometry.subvol_center[:, self.slice_axis], self.subvol_temperature
            )
            self.temperatures = self.temperature_interpolator(
                self.positions[:, self.slice_axis]
            )
        elif geometry.subvol_type == "grid" and np.any(geometry.grid == 1):
            self.temperature_interpolator = self.temp_interp(
                geometry.subvol_center[:, geometry.grid != 1], self.subvol_temperature
            )
            self.temperatures = self.temperature_interpolator(
                self.positions[:, geometry.grid != 1]
            )
        else:
            self.temperature_interpolator = self.temp_interp(
                geometry.subvol_center, self.subvol_temperature
            )
    ...
    def calculate_energy(self, phonon):
        ...
        if self.T_reference == "local":
            dn = self.occupation - phonon.calculate_occupation(
                self.subvol_temperature[self.subvol_id], self.omega
            )
            ref = phonon.crystal_energy_function(self.subvol_temperature)
        else:
            dn = (
                self.occupation
                - self.reference_occupation[self.modes[:, 0], self.modes[:, 1]]
            )
    ...
    def calculate_heat_flux(self, phonon):
        ...
        for i in range(self.n_of_subvols):
            ...
            heat_flux[i, :] = np.sum(
                self.group_vel[ind, :] * self.energies[ind].reshape(-1, 1), axis=0
            )
    ...
    def calculate_kappa(self, geometry):
        if geometry.subvol_type == "slice":
            ...
            dx = (
                2
                * geometry.bounds[:, geometry.slice_axis].ptp()
                * self.a_in_m
                / self.n_of_subvols
            )
            dT = T[2:] - T[:-2]
            ...
            phi = (
                self.subvol_heat_flux[i, :] + self.subvol_heat_flux[j, :]
            ) / 2  # (C,3)

            phi_dot_n = np.sum(phi * n, axis=1)

            with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
                self.svcon_kappa = np.where(
                    dT == 0,
                    0,
                    -phi_dot_n * np.linalg.norm(dx, axis=1) * self.a_in_m / dT,
                )
    ...
    def timesteps_to_boundary(self, positions, velocities, geometry):
        ...
        while start < positions.shape[0]:
            (
                ...
                new_ts_to_boundary,
                new_index_facets,
            ) = geometry.mesh.find_boundary(
                positions[start : start + stride, :],
                velocities[start : start + stride, :],
            )  # find collision in true boundary

            new_ts_to_boundary /= self.dt

            collision_pos = np.concatenate((collision_pos, new_collision_pos), axis=0)
            index_facets = np.concatenate(
                (index_facets, new_index_facets), axis=0, dtype=int
            )

            ts_to_boundary = np.concatenate(
                (ts_to_boundary, new_ts_to_boundary), axis=0, dtype=float
            )
    ...
    def calculate_fbz_specularity(self, geometry, phonon):
        ...
        ):  # to stop it giving errors when ||v|| = 0
        ...
        self.specularity = np.exp(
            self.specularity_constant * (k_norm**2)
        )  # (F, Q, J)
    ...
    def diffuse_scat_probability(self, geometry, phonon):
        ...
        if self.rough_facets.shape[0] == 0:
            ...
            self.creation_roulette = np.zeros(
                (
                    self.rough_facets.shape[0],
                    phonon.number_of_qpoints * phonon.number_of_branches,
                )
            )

            C_total = np.where(
                v_dot_n > 0, v_dot_n, 0
            )  # total rate of creation    (F, Q, J)
            D_total = np.where(
                v_dot_n < 0, -v_dot_n, 0
            )  # total rate of destruction (F, Q, J)
            ...
            self.creation_rate = np.where(
                np.isnan(self.creation_rate), 0, self.creation_rate
            )

            unique_n, inv_n = np.unique(
                n, axis=0, return_inverse=True
            )  # getting unique normals of rough facets

            for i_n, u_n in enumerate(unique_n):
                i_f = (inv_n == i_n).nonzero()[0]  # facets with that normal
                corr_indices = (np.linalg.norm(corr_n - u_n, axis=1) < 1e-10).nonzero()[
                    0
                ]

                # unique outward modes and counts to find repetitions
                corr = self.correspondent_modes[corr_indices, :]
                unique_out, inv_out, count_out = np.unique(
                    corr[:, [0, 1, 2, 5, 6]],
                    axis=0,
                    return_counts=True,
                    return_inverse=True,
                ...
                once = np.isin(inv_out, (count_out == 1).nonzero()[0]).nonzero()[
                    0
                ]  # modes originated only by one specular reflection
                more_unique = (count_out > 1).nonzero()[
                    0
                ]  # modes originated by more than one specular reflection

                for ii_f in i_f:
                    self.creation_rate[
                        ii_f, out_q[corr_indices][once], out_j[corr_indices][once]
                    ] -= specular_D[
                        ii_f, in_q[corr_indices][once], in_j[corr_indices][once]
                    ]  # remove the creation due to specular scattering

                    for mu in more_unique:  # for each repeated outward mode
                        repeated = (inv_out == mu).nonzero()[0]
                        self.creation_rate[
                            ii_f,
                            unique_out[mu, 3].astype(int),
                            unique_out[mu, 4].astype(int),
                        ] -= specular_D[
                            ii_f,
                            in_q[corr_indices][repeated],
                            in_j[corr_indices][repeated],
                        ].sum()
            ...
            for i_f, _ in enumerate(self.rough_facets):
                self.creation_roulette[i_f, :] = (
                    np.cumsum(self.creation_rate[i_f, :, :])
                    / np.cumsum(self.creation_rate[i_f, :, :]).max()
                )

    def select_reflected_modes(
        self, in_modes, col_fac, col_pos, n_in, omega_in, geometry, phonon
    ):
        ...
        if np.any(indexes_spec):
            a = np.hstack(
                (
                    -geometry.facets_normal[col_fac[indexes_spec], :],
                    in_modes[indexes_spec, :],
                )
            )
        ...
        if np.any(indexes_diff):
            out_modes[indexes_diff, :] = self.pick_diffuse_modes(
                col_fac[indexes_diff], phonon
            )

            omega_out[indexes_diff] = phonon.omega[
                out_modes[indexes_diff, 0], out_modes[indexes_diff, 1]
            ]
            ...
            if geometry.subvol_type == "slice" and self.temp_interp_type in [
                ...
                T_diff = self.temperature_interpolator(
                    col_pos[indexes_diff, self.slice_axis]
                )
            elif geometry.subvol_type == "grid" and np.any(geometry.grid == 1):
            ...
            n_out[indexes_diff] = phonon.calculate_occupation(
                T_diff, omega_out[indexes_diff]
            )
    ...
    def find_degeneracies(self, phonon):
        ...
        possible_degen = (
            np.absolute(
                np.transpose(np.expand_dims(phonon.omega, 2), (1, 0, 2)) - phonon.omega
            )
            < 1e-10
        )  # same k and omega
        possible_degen[
            np.arange(phonon.number_of_branches),
            :,
            np.arange(phonon.number_of_branches),
        ] = False  # removing self references

        deg_modes = np.vstack(possible_degen.nonzero()).T  # getting q and j
        deg_modes = deg_modes[:, [1, 0, 2]]  # flipping to make it as [q, j1, j2]

        deg_modes = deg_modes[
            np.lexsort(keys=(deg_modes[:, 2], deg_modes[:, 1], deg_modes[:, 0])), :
        ]  # sorting just to keep it neat
        ...
        while i < n:
            deg_modes = deg_modes[
                ~np.all(deg_modes == deg_modes[i, [0, 2, 1]], axis=1), :
            ]
            i += 1
            n = deg_modes.shape[0]
        ...
        self.degen_index[deg_modes[:, 0], deg_modes[:, 1]] = np.arange(
            deg_modes.shape[0]
        )
        self.degen_index[deg_modes[:, 0], deg_modes[:, 2]] = np.arange(
            deg_modes.shape[0]
        )
    ...
    def find_specular_correspondences(self, geo, phonon):
        ...
        true_spec = np.zeros(
            (facets.shape[0], phonon.number_of_qpoints, phonon.number_of_branches),
            dtype=bool,
        )

        correspondent_modes = np.zeros((0, 7))

        k_grid = phonon.q_to_k(
            np.absolute(1 / (2 * phonon.data_mesh))
        )  # size of the grid in each direction

        delta_omega = (
            np.sum((phonon.group_vel * k_grid) ** 2, axis=2) ** 0.5
        )  # the acceptable variation of omega from the central value for each mode
        ...
        for i_n, n in enumerate(normals):
            ...
            norm = np.fmax(
                sorted_norm_out[right_i[~far_right]], sorted_norm_in[~far_right]
            )

            far_right[~far_right] = (
                sorted_vx_out[right_i[~far_right]] - sorted_vx_in[~far_right]
            ) / norm > crit
            done = np.copy(far_right)
            while not np.all(done):
                done[~done] = (
                    right_i[~done] == sorted_vx_out.shape[0] - 1
                )  # check the ones not done are at the border and mark them as done

                norm = np.fmax(
                    sorted_norm_out[right_i[~done] + 1], sorted_norm_in[~done]
                )

                take_step = (
                    sorted_vx_out[right_i[~done] + 1] - sorted_vx_in[~done]
                ) / norm < crit  # check if the ones not done can take a step

                ind = (~done).nonzero()[0][
                    take_step
                ]  # indices of the ones that will walk
            ...
            norm = np.fmax(
                sorted_norm_out[left_i[~far_left] - 1], sorted_norm_in[~far_left]
            )

            far_left[~far_left] = (
                sorted_vx_in[~far_left] - sorted_vx_out[left_i[~far_left]]
            ) / norm > crit
            ...
            while not np.all(done):
                ...
                norm = np.fmax(
                    sorted_norm_out[left_i[~done] - 1], sorted_norm_in[~done]
                )

                take_step = (
                    sorted_vx_in[~done] - sorted_vx_out[left_i[~done] - 1]
                ) / norm < crit  # check if the ones not done can take a step
            ...
            L = (
                right_i - left_i + 1
            )  # how many possible reflections for each incoming mode

            possible_reflections = np.zeros(
                (L.sum(), 2), dtype=int
            )  # initialise possible reflections array
            ...
            for i in range(sorted_vx_in.shape[0]):  # for each SORTED vx_in
                if L[i] > 0:
                    ...
                    possible_reflections[N : N + L[i], 1] = sorted_i_out[
                        left_i[i] : right_i[i] + 1
                    ]
                    N += L[i]

            ref_norm = np.fmax(
                v_in_norm[possible_reflections[:, 0]],
                v_out_norm[possible_reflections[:, 1]],
            )
            # DIRECTION Y
            diff = np.absolute(
                v_in[possible_reflections[:, 0], 1]
                - v_out[possible_reflections[:, 1], 1]
            )
            ...
            # DIRECTION Z
            diff = np.absolute(
                v_in[possible_reflections[:, 0], 2]
                - v_out[possible_reflections[:, 1], 2]
            )
            possible_reflections = possible_reflections[diff / ref_norm < crit, :]

            diff = np.absolute(
                omega_in[possible_reflections[:, 0]]
                - omega_out[possible_reflections[:, 1]]
            )
            delta = (
                delta_omega_in[possible_reflections[:, 0]]
                + delta_omega_out[possible_reflections[:, 1]]
            )
            ...
            fig, ax = plt.subplots(
                nrows=3, ncols=3, figsize=(15, 15), dpi=100, sharex="row", sharey="row"
            )
            ...
            plt.suptitle(
                "Normal = <{:.2f}, {:.2f}, {:.2f}>. Angles = {:.2f}, {:.2f}, {:.2f}".format(
                    n[0],
                    n[1],
                    n[2],
            ...
            correspondent_modes = np.vstack(
                (
                    correspondent_modes,
                    np.vstack(
                        (
                            np.ones(n_ts) * n.reshape(-1, 1),
                            spec_q_in,
                            spec_j_in,
                            spec_q_out,
                            spec_j_out,
        ...
        self.specular_function = NearestNDInterpolator(
            self.correspondent_modes[:, :-2], self.correspondent_modes[:, -2:]
        )
        self.true_specular = copy.copy(true_spec)
        self.specularity = self.true_specular.astype(int) * self.specularity

        np.savetxt(
            os.path.join(self.results_folder_name, "specular_correspondences.txt"),
            self.correspondent_modes,
            fmt="%.3f %.3f %.3f %d %d %d %d",
        )

    def reservoir_boundary_condition(
        self,
        positions,
        group_velocities,
        collision_facets,
        ...
        new_modes, new_occupation, new_omega = self.select_reservoir_modes(
            collision_facets,
            omega_in,
            occupation_in,
            group_velocities,
        ...
        ) = self.timesteps_to_boundary(new_positions, new_group_vel, geometry)
    ...
    def periodic_boundary_condition(
        self,
        positions,
        group_velocities,
        collision_facets,
        ...
        collision_facets = collision_facets.astype(
            int
        )  # ensuring collision facets are integers

        # getting which face is connected
        rows = np.array(
            [np.where(i == self.connected_facets)[0][0] for i in collision_facets]
        )
        mask = self.connected_facets[rows, :] == collision_facets.reshape(-1, 1)
        connected_facets = (
            (self.connected_facets[rows, :] * ~mask).sum(axis=1).astype(int)
        )

        previous_positions = copy.deepcopy(
            positions
        )  # the path of some particles is calculated from their current position
        first = np.nonzero(calculated_ts == 0)[
            0
        ]  # but for those at the beginning of the timestep
        previous_positions[first, :] -= (
            group_velocities[first, :] * self.dt
        )  # their starting position is the one before the collision

        L = (
            geometry.facet_centroid[connected_facets, :]
            - geometry.facet_centroid[collision_facets, :]
        )  # get translation vector between connected facets
        ...
        (
            ...
            new_collision_facets,
            new_collision_positions,
        ) = self.timesteps_to_boundary(new_positions, group_velocities, geometry)

        calculated_ts += np.linalg.norm(
            (collision_positions - previous_positions), axis=1
        ) / np.linalg.norm((group_velocities * self.dt), axis=1)
    ...
    def roughness_boundary_condition(
        self,
        positions,
        group_velocities,
        collision_facets,
        ...
        # select new modes and get their properties
        new_modes, new_occupation, new_omega = self.select_reflected_modes(
            in_modes,
            collision_facets,
            collision_positions,
            occupation_in,
    ...
    def boundary_scattering(self, geometry, phonon):
        ...
        calculated_ts = np.ones(
            indexes_all.shape
        )  # start the tracking of calculation as 1 (N_p,)
        ...
        while np.any(calculated_ts < 1):
            ...
            indexes_res = np.logical_and(
                calculated_ts < 1, np.in1d(self.collision_cond, ["T", "F"])
            )
            indexes_res = np.logical_and(
                indexes_res, (1 - calculated_ts) > new_n_timesteps
            )
            ...
            if np.any(indexes_res):
                ...
                ) = self.reservoir_boundary_condition(
                    self.positions[indexes_res, :],
                    self.group_vel[indexes_res, :],
                    self.collision_facets[indexes_res],
                    self.collision_positions[indexes_res, :],
            ...
            indexes_per = np.logical_and(
                calculated_ts < 1, self.collision_cond == "P"  #
            )
            indexes_per = np.logical_and(
                indexes_per, (1 - calculated_ts) > new_n_timesteps
            )
            ...
            indexes_ref = np.logical_and(calculated_ts < 1, self.collision_cond == "R")
            indexes_ref = np.logical_and(
                indexes_ref, (1 - calculated_ts) > new_n_timesteps
            )
            ...
            if np.any(indexes_ref):
                ...
                ) = self.roughness_boundary_condition(
                    self.positions[indexes_ref, :],
                    self.group_vel[indexes_ref, :],
                    self.collision_facets[indexes_ref],
                    self.collision_positions[indexes_ref, :],
            ...
            indexes_drift = np.logical_and(
                calculated_ts < 1, (1 - calculated_ts) < new_n_timesteps
            )

            if np.any(indexes_drift):
                self.positions[indexes_drift, :] += (
                    self.group_vel[indexes_drift, :]
                    * self.dt
                    * (1 - calculated_ts[indexes_drift]).reshape(-1, 1)
                )
    ...
    def adjust_reservoir_balance(self, geometry, phonon):
        if self.n_of_reservoirs > 0:
            ...
            self.res_heat_flux *= phonon.number_of_active_modes / (
                self.particle_density
                * self.dt
                * self.n_dt_to_conv
                * geometry.facets_area[self.res_facet].reshape(-1, 1)
            )
            ...
            self.res_energy_balance *= phonon.number_of_active_modes / (
                self.particle_density * self.dt * self.n_dt_to_conv
            )
            self.res_energy_balance = phonon.normalise_to_density(
                self.res_energy_balance
            )  # eV/ps
    ...
    def lifetime_scattering(self, phonon):
        ...
        n0 = phonon.calculate_occupation(
            self.temperatures, self.omega
        )  # calculating Bose-Einstein occcupation

        with np.errstate(
            divide="ignore", invalid="ignore", over="ignore"
        ):  # this case is treated inside np.where, but the warning still keeps showing up
            self.occupation = np.where(
                tau > 0, n0 + (self.occupation - n0) * np.exp(-self.dt / tau), n0
            )  # n = n0 + (n - n0(T)) e^(-dt/tau(T))
    ...
    def contains_check(self, geometry):
        ...
        out = np.logical_or(
            np.any(self.positions < geometry.bounds[0, :] - 1e-10, axis=1),
            np.any(self.positions > geometry.bounds[1, :] + 1e-10, axis=1),
        ).nonzero()[0]

        if out.shape[0] > 0:
            self.positions[out, :] = geometry.mesh.sample_volume(out.shape[0])
            (
                self.n_timesteps[out],
                self.collision_facets[out],
                self.collision_positions[out],
            ) = self.timesteps_to_boundary(
                self.positions[out, :], self.group_vel[out, :], geometry
            )
            self.collision_cond[out] = self.get_collision_condition(
                self.collision_facets[out]
            )
    ...
    def run_timestep(self, geometry, phonon):
        ...
        if (self.current_timestep % 100) == 0:
            ...
            info = "Timestep {:>5d} - max residue: {:>9.3e} ({:<9s}) [".format(
                int(self.current_timestep), self.max_residue, self.max_residue_qt
            )
            for sv in range(self.n_of_subvols):
        ...
        self.boundary_scattering(
            geometry, phonon
        )  # perform boundary scattering/periodicity and particle deletion
    ...
    def initialise_residue(self, geo):
        if geo.subvol_type == "slice":
            ...
            self.old_mean_large = np.ones(
                4 * self.n_of_subvols + self.n_of_reservoirs + geo.n_of_subvol_con
            )
            self.old_std_large = np.ones(
                4 * self.n_of_subvols + self.n_of_reservoirs + geo.n_of_subvol_con
            )
        ...
        if geo.subvol_type == "slice":
            ...
            self.residue_qts = (
                ["T_{:d}".format(i) for i in range(self.n_of_subvols)]
            ...
            self.residue_qts = (
                ["T_{:d}".format(i) for i in range(self.n_of_subvols)]
                + [
                    "phi_{:s}_{:d}".format(i, j)
                    for j in range(self.n_of_subvols)
                    for i in ["x", "y", "z"]
                ]
                + ["en_res_{:d}".format(i) for i in range(self.n_of_reservoirs)]
    ...
    def update_residue(self, geo):
        ...
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            ...
            if geo.subvol_type == "slice":
                new_mean_large = np.concatenate(
                    (
                        self.view.mean_T,
                        self.view.mean_sv_phi[
                            3 * np.arange(self.n_of_subvols) + self.slice_axis
                        ],
                        self.view.mean_en_res,
                        self.view.mean_sv_k,
                    )
                )
                new_std_large = np.concatenate(
                    (
                        self.view.std_T,
                        self.view.std_sv_phi[
                            3 * np.arange(self.n_of_subvols) + self.slice_axis
                        ],
                        self.view.std_en_res,
                        self.view.std_sv_k,
                    )
                )
            else:
            ...
            residue_mean = np.absolute(
                (new_mean_large - self.old_mean_large) / self.old_mean_large
            )

            # residue_std = np.absolute((new_std_large - self.old_std_large)/self.old_std_large)

        self.residue_all = np.where(
            new_std_large > np.absolute(new_mean_large), 0, residue_mean
        )
    ...
    def plot_figures(self, geometry, phonon, property_plot=["energy"], colormap="jet"):
        ...
        fig, ax = geometry.mesh.plot_facet_boundaries(
            l_color=self.view.ax_style["axiscolor"]
        )
        n = len(property_plot)
        ...
        for i in range(n):
            if property_plot[i] in ["T", "temperature", "temperatures"]:
                ...
                if self.n_of_reservoirs > 0:
                    T = np.concatenate(
                        (
                            self.res_bound_values[self.res_bound_cond == "T"],
                            self.subvol_temperature,
                        )
                    )
                else:
                ...
                emin = (
                    phonon.calculate_energy(T.min() - 1, phonon.omega)
                    - phonon.calculate_energy(T.mean(), phonon.omega)
                ).min()
                emax = (
                    phonon.calculate_energy(T.max() + 1, phonon.omega)
                    - phonon.calculate_energy(T.mean(), phonon.omega)
                ).max()

                order = [
                    np.floor(np.log10(np.absolute(emin))),
                    np.floor(np.log10(np.absolute(emax))),
                ]
            ...
            if i == 0:
                ...
                cb = fig.colorbar(
                    matplotlib.cm.ScalarMappable(norm=norm, cmap=self.colormap),
                    ax=ax,
                    location="bottom",
            ...
            cb.set_ticks(ticks)
            cb.set_ticklabels(
                [format.format(i) for i in ticks], size="x-small", color=linecolor
            )
    ...
    def open_convergence(self, geometry):
        ...
        n_dt_to_conv = (
            np.floor(np.log10(self.args.iterations[0])) - 2
        )  # number of timesteps to save convergence data
    ...
    def write_convergence(self, geometry):
        ...
        if self.n_of_reservoirs > 0:
            line += (
                np.array2string(
                    self.res_energy_balance,
                    formatter={"float_kind": "{:>12.5e}".format},
                ).strip("[]")
                + " "
            )
            for i in range(self.n_of_reservoirs):
                line += (
                    np.array2string(
                        self.res_heat_flux[i, :],
                        formatter={"float_kind": "{:>14.6e}".format},
                    ).strip("[]")
                    + " "
                )
        ...
        line += (
            np.array2string(
                self.subvol_temperature, formatter={"float_kind": "{:>9.3f}".format}
            ).strip("[]")
            + " "
        )
        line += (
            np.array2string(
                self.subvol_energy, formatter={"float_kind": "{:>12.5e}".format}
            ).strip("[]")
            + " "
        )
        ...
        line += (
            np.array2string(
                self.subvol_N_p.astype(int), formatter={"int": "{:>10d}".format}
            ).strip("[]")
            + " "
        )
        ...
        if geometry.subvol_type == "slice":
            ...
            line += (
                np.array2string(
                    self.subvol_kappa, formatter={"float_kind": "{:>12.5e}".format}
                ).strip("[]")
                + " "
            )
            ...
            line += (
                np.array2string(
                    self.svcon_kappa, formatter={"float_kind": "{:>14.7e}".format}
                ).strip("[]")
                + " "
            )
    ...
    def write_final_state(self, geometry):
        ...
        header = (
            "Particles final state data \n"
            + "Date and time: {}\n".format(time)
            + "hdf file = {}, POSCAR file = {}\n".format(
                self.args.hdf_file, self.args.poscar_file
            )
            + "q-point, branch, pos x [angs], pos y [angs], pos z [angs], occupation"
        )

        data = np.hstack((self.modes, self.positions, self.occupation.reshape(-1, 1)))
        ...
        if self.current_timestep > 0:
            ...
            if geometry.subvol_type == "slice":
                ...
                data = np.hstack(
                    (
                        np.arange(self.n_of_subvols).reshape(-1, 1),
                ...
                np.savetxt(
                    filename,
                    data,
                    "%d, %.3e, %.3e, %.3e, %.3f, %.3e, %.3e, %.3e, %.3e, %.3e, %.3e, %.3e, %.3e, %.3e",
                    delimiter=",",
                    header=header,
                )
                ...
                np.savetxt(
                    filename,
                    data,
                    "%d, %.3e, %.3e, %.3e, %.3f, %.3e, %.3e, %.3e, %.3e, %.3e, %.3e, %.3e",
                    delimiter=",",
                    header=header,
                )
                ...
                filename = os.path.join(
                    self.results_folder_name, "subvol_connections.txt"
                )

                header = (
                    "connections final state data \n"
                    + "Date and time: {}\n".format(time)
                    + "hdf file = {}, POSCAR file = {}\n".format(
                        self.args.hdf_file, self.args.poscar_file
                    )
                ...
                data = np.hstack(
                    (
                        np.arange(geometry.n_of_subvol_con).reshape(-1, 1),
                ...
                np.savetxt(
                    filename,
                    data,
                    "%d, %d, %d, %.3e, %.3e, %.3e, %.3f, %.3e, %.3e, %.3e, %.3e, %.3e",
                    delimiter=",",
                    header=header,
                )
