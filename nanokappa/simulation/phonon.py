...
#   - Think in how to get the direct or cartesian coordinates of atoms positions
# from POSCAR file.
...
class Phonon(Constants):
    ...
    def get_mat_folder(self):
        if len(self.args.mat_folder) > 0:
            ...
            # I don't know why argparse converts a single argument to a list
            self.mat_folder = os.path.relpath(self.args.mat_folder[0])
        else:
            self.mat_folder = ""
    ...
    def load_base_properties(self):
        ...
        poscar_file = os.path.join(
            self.mat_folder, self.args.poscar_file[0]
        )  # I don't know why argparse converts a single argument to a list
        ...
        hdf_file = os.path.join(
            self.mat_folder, self.args.hdf_file[0]
        )  # I don't know why argparse converts a single argument to a list
        ...
        self.load_frequency()
        qpoints_FBZ, frequency = self.expand_FBZ(
            0,
            self.weights,
            self.q_points,
            self.frequency,
        ...
        self.load_group_vel()
        qpoints_FBZ, group_vel = self.expand_FBZ(
            0,
            self.weights,
            self.q_points,
            self.group_vel,
        ...
        self.load_gamma()
        qpoints_FBZ, gamma = self.expand_FBZ(
            1, self.weights, self.q_points, self.gamma, 0, rotations, reciprocal_lattice
        )
        self.gamma = gamma
        ...
        self.number_of_active_modes = (
            self.number_of_modes - self.number_of_inactive_modes
        )
        ...
        self.unique_modes = (
            np.stack(
                np.meshgrid(
                    np.arange(self.number_of_qpoints),
                    np.arange(self.number_of_branches),
                ),
                axis=-1,
            )
            .reshape(-1, 2)
        ...
        print(
            "Material info: {:d} q-points; {:d} branches -> {:d} modes in total.".format(
                self.number_of_qpoints, self.number_of_branches, self.number_of_modes
            )
        )
    ...
    def load_hdf_data(self, hdf_file):
        ...
        self.data_mesh = np.array(
            self.data_hdf["mesh"]
        )  # mesh discretisation points in each dimension
    ...
    def load_q_points(self):
        ...
        self.q_points = np.array(
            self.data_hdf["qpoint"]
        )  # reduced reciprocal coordinates
    ...
    def plot_FBZ(self):
        fig, ax = plt.subplots(
            nrows=1, ncols=1, subplot_kw={"projection": "3d"}, figsize=(6, 5), dpi=300
        )
        ax.scatter(
            self.wavevectors[:, 0],
            self.wavevectors[:, 1],
            self.wavevectors[:, 2],
            s=1,
            c=np.sum(self.wavevectors**2, axis=1),
        )
    ...
    def find_min_k(self, k, return_disp=False):
        ...
        while np.any(active):  # For reference: Qa = number of qpoints still active
            ...
            )  # minimum norm for each mode (Qa, 1)
            i_min = np.argmax(
                k_norm == k_min, axis=1
            )  # get the index where the minimum is located (Qa,)

            if return_disp:
                disp[active, :] += n[
                    i_min, 0, :
                ]  # displacement from previous k in reduced coordinates

            q[active, :] = np.copy(q_new[i_min, np.arange(active.sum()), :])

            active[active] = ~(
                i_min == i0
            )  # If the minimum is in i0, the shortest wavevector was found for that mode
    ...
    def calculate_lifetime(self):
        ...
        self.lifetime = np.where(
            self.gamma > 0, 1 / (2 * 2 * np.pi * self.gamma), 0
        )  # ps
    ...
    def calculate_occupation(self, T, omega):
        ...
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            occupation = np.where(
                ~flag, 0, 1 / (np.exp(omega * self.hbar / (T * self.kb)) - 1)
            )
    ...
    def calculate_crystal_energy(self, T):
        ...
        crystal_energy = (
            self.calculate_energy(T, self.omega) * ~self.inactive_modes_mask
        ).sum(
            axis=(1, 2)
        )  # eV - energy sum of all modes
        crystal_energy = self.normalise_to_density(
            crystal_energy
        )  # eV / a³ - normalising to density
    ...
    def initialise_temperature_function(self):
        ...
        # Energy array
        self.energy_array = np.array(
            list(map(self.calculate_crystal_energy, T_array))
        ).reshape(-1)

        # Interpolating
        self.temperature_function = interp1d(
            self.energy_array,
            T_array,
            kind="linear",
        ...
        self.crystal_energy_function = interp1d(
            T_array,
            self.energy_array,
            kind="linear",
            fill_value=(self.energy_array.min(), self.energy_array.max()),
            bounds_error=False,
        )
    ...
    def find_degeneracies(self):
        ...
        for q in range(self.number_of_qpoints):  # for each wavevector (q-point)
            u_omega, i_omega, c_omega = np.unique(
                self.omega[q, :], return_inverse=True, return_counts=True
            )  # unique frequencies, inverse and their counts

            if np.any(c_omega > 1):  # if there is any repeated omega
                ui_omega = np.nonzero(c_omega > 1)[
                    0
                ]  # unique indexes of the degenerate branches
                for ui in ui_omega:  # for each degenerate branch
                    ...
                    u_v, i_v, c_v = np.unique(
                        v, axis=0, return_inverse=True, return_counts=True
                    )  # unique velocities, inverse and counts

                    if np.any(c_v > 1):
                        ui_v = np.nonzero(c_v > 1)[0]
                        for uui in ui_v:
                            d_q = (np.ones(c_v[uui]) * q).astype(
                                int
                            )  # degenerate qpoints
    ...
    def plot_relaxation_time(self):
        ...
        T_all = self.temperature_array[
            self.temperature_array % 100 == 0
        ]  # multiples of 100 K available in the data
        ...
        for i, T in enumerate(T_all):
            ...
            Tqj = np.hstack(
                (
                    (np.ones(self.unique_modes.shape[0]) * T).reshape(-1, 1),
                    self.unique_modes,
                )
            )
        ...
        handles = [
            Ellipse(
                xy=np.random.rand(2)
                * np.array(
                    [np.array(ax.get_xlim()).ptp(), np.array(ax.get_ylim()).ptp()]
                )
                + np.array([ax.get_xlim()[0], ax.get_ylim()[0]]),
                height=1,
                width=1,
                color=c,
                label="{:.1f} K".format(T_all[i]),
            )
        ...
        ax.set_xlabel(r"Angular frequency $\omega$ [rad THz]", fontsize="large")
        ax.set_ylabel(r"Phonon relaxation time $\tau$ [ps]", fontsize="large")
    ...
    def plot_density_of_states(self):
        ...
        ax.stackplot(
            centers,
            dos.T,
            labels=["Branch {:d}".format(i) for i in range(self.number_of_branches)],
            step="pre",
            colors=colors,
        )
        ...
        plt.title(
            r"Density of states. {:d} bins, $d\omega = $ {:.3f} THz".format(
                n_bins, d_omega
            ),
            fontsize="large",
        )
    ...
    def expand_FBZ(
        self, axis, weight, qpoints, tensor, rank, rotations, reciprocal_lattice
    ):
        ...
        for i, q in enumerate(qpoints):
            ...
            for r in rotations:
                ...
                r_cart = np.dot(
                    reciprocal_lattice, np.dot(r, np.linalg.inv(reciprocal_lattice))
                )
            ...
            star_q, return_index, return_inverse, return_counts = np.unique(
                star_mul,
                return_index=True,
                return_inverse=True,
                return_counts=True,
