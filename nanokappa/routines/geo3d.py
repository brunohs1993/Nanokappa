...
def line_triangle_intersection(lo, ld, v, return_t=False):
    ...
    if return_t:
        c, t = line_plane_intersection(
            lo, ld, n, v[0, :], return_t=True
        )  # find intersection point
    else:
        # find intersection point
        c = line_plane_intersection(lo, ld, n, v[0, :])
...
def transform_3d_to_2d(points, normal, origin=None, b1=None, b2=None):
    ...
    Returns the coordinates of the points projected onto the plane and the base
    vectors.
    """
    ...
    if b1 is None or b2 is None:
        ...
        # make b1 orthogonal to the normal
        b1 = b1 - normal * np.sum(normal * b1)
    ...
    for p in points:  # for each vertex
        ...
        plane_coord = np.vstack(
            (plane_coord, np.linalg.solve(A, B).T[0, :2])
        )  # store the plane coordinates

    return plane_coord, b1, b2


def triangulate_plane(v, b=None):
    """Triangulate given vertices by taking the first three vertices as
    reference of the plane.

    The other points will be projected onto this plane.
    Returns the new vertices and the vertex indices for each triangle.
    b defines the boundaries (rings) of the geometry. If b is not informed, no
