...
class FaceGroup(BaseModel, frozen=True):
    faces: list[Triangle]
    ...
    def graph(self) -> list:
        graph = nx.Graph()
        graph.add_nodes_from([i for i in range(len(self.faces))])
        for i, fe1 in enumerate(self.faces_edges[:-1]):
            for j, fe2 in enumerate(self.faces_edges[i + 1:]):
                _, counts = np.unique([fe1, fe2], return_counts=True)
                if np.any(counts == 2):
                    graph.add_edge(i, j)
        return graph
