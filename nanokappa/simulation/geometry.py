...
class Geometry:
    def __init__(self, args):
        ...
        self.check_facet_connections(
            args
        )  # check if all connections are valid and adjust vertices
        self.plot_mesh_bc()
    ...
    def load_geo_file(self, shape):
        ...
        if shape in self.standard_shapes:
            ...
            self.mesh = Mesh(
                np.around(prev_mesh.vertices, decimals=10), prev_mesh.faces
            )
    ...
    def generate_primitives(self, shape, dims):
        if shape in ["cuboid", "box"]:
            ...
            base_faces = np.array(
                [[i, i + 1, 0] if i < Ns else [i, 1, 0] for i in range(1, Ns + 1)]
            )
            ...
            for i in range(1, Nc + 1):
                vertices = np.vstack(
                    (
                        vertices,
                        ring
                        + np.array([int(i % 2 == 1) * dx, int(i % 2 == 1) * dy, i * L]),
                    )
                )

                faces = np.vstack((faces, side_faces + (i - 1) * Ns + 1))

            # closing
            vertices = np.vstack(
                (vertices, np.array([(Nc % 2 == 1) * dx, (Nc % 2 == 1) * dy, Nc * L]))
            )
            ...
            faces = np.vstack(
                (
                    ...
                    np.where(
                        base_faces == 0,
                        vertices.shape[0] - 1,
                        base_faces + vertices.shape[0] - Ns - 2,
                    ),
                )
            )
            ...
            outer_ring = (
                np.vstack((np.cos(angles), np.sin(angles), np.zeros(Ns))) * R
            ).T
            inner_ring = (
                np.vstack((np.cos(angles), np.sin(angles), np.zeros(Ns))) * r
            ).T
            ...
            for i in range(1, Nc + 1):
                if i % 2 == 1:
                    vertices = np.vstack(
                        (vertices, inner_ring + np.array([0, 0, i * L]))
                    )
                else:
                    vertices = np.vstack(
                        (vertices, outer_ring + np.array([0, 0, i * L]))
                    )

                faces = np.vstack((faces, side_faces + (i - 1) * Ns + 1))

            # closing
            vertices = np.vstack((vertices, np.array([0, 0, Nc * L])))
            ...
            if R <= r:
                raise Exception(
                    "Outer radius smaller or equal to the inner radius. Check parameters."
                )
            ...
            inner_ring = (
                np.vstack((np.cos(inner_angles), np.sin(inner_angles), np.zeros(Ns)))
                * r
            ).T
            outer_ring = (
                np.vstack((np.cos(outer_angles), np.sin(outer_angles), np.zeros(Ns)))
                * R
            ).T
            ...
            if s:  # if start with large section
                ...
                vertices = np.vstack(
                    (np.zeros(3), inner_ring, inner_ring + np.array([0, 0, l]))
                )
                faces = np.vstack((small_lid_faces, side_faces + 1))
            ...
            for i in range(1, Nc):
                if section == "small":
                    z += l
                    vertices = np.vstack((vertices, inner_ring + np.array([0, 0, z])))

                    faces = np.vstack((faces, side_faces + vertices.shape[0] - 3 * Ns))
            ...
            # closing
            vertices = np.vstack((vertices, np.array([0, 0, z])))
            if section == "small":
                faces = np.vstack(
                    (
                        faces,
                        np.where(
                            small_lid_faces == 0,
                            vertices.shape[0] - 1,
                            small_lid_faces + vertices.shape[0] - 2 * Ns - 2,
                        ),
                    )
                )
            if section == "large":
                faces = np.vstack(
                    (
                        ...
                        np.where(
                            small_lid_faces == 0,
                            vertices.shape[0] - 1,
                            small_lid_faces + vertices.shape[0] - Ns - 2,
                        ),
                    )
                )
            ...
            inner_ring = (
                np.vstack((np.cos(inner_angles), np.sin(inner_angles), np.zeros(N))) * r
            ).T
            outer_ring = (
                np.vstack((np.cos(outer_angles), np.sin(outer_angles), np.zeros(N))) * R
            ).T

            vertices = np.vstack((np.zeros(3), inner_ring, outer_ring))
            vertices = np.vstack((vertices, vertices + np.array([0, 0, H])))
            ...
            for i in range(N):
                if i == N - 1:
                    lid = np.vstack(
                        (lid, np.array([0, i + 1, 1]), np.array([i + 1 + N, i + 1, 1]))
                    )
            ...
            faces = np.vstack((lid, side, lid + 2 * N + 1))

        elif shape in ["freewire"]:

            R = np.array(
                [dims[i] for i in range(0, len(dims) - 1, 2)], dtype=float
            )  # radii
            L = np.array(
                [dims[i] for i in range(1, len(dims) - 1, 2)], dtype=float
            )  # section lengths
            ...
            base_faces = np.array(
                [[i, i + 1, 0] if i < N else [i, 1, 0] for i in range(1, N + 1)]
            )
            ...
            for i, r in enumerate(R[1:]):
                vertices = np.vstack(
                    (vertices, ring * r + np.array([0, 0, L[: i + 1].sum()]))
                )

                faces = np.vstack((faces, side_faces + i * N + 1))

            # closing
            vertices = np.vstack(
                (vertices, np.array([0, 0, L[: (R.shape[0] - 1)].sum()]))
            )
            ...
            faces = np.vstack(
                (
                    ...
                    np.where(
                        base_faces == 0,
                        vertices.shape[0] - 1,
                        base_faces + vertices.shape[0] - N - 2,
                    ),
                )
            )
    ...
    def set_subvolumes(self):
        ...
        if self.subvol_type == "slice":
            ...
            self.subvol_center = self.subvol_center[
                np.lexsort(
                    (
                        self.subvol_center[:, 2],
                        self.subvol_center[:, 1],
                        self.subvol_center[:, 0],
                    )
                )
            ]  # sort it

            self.slice_length = (
                np.ptp(self.bounds[:, self.slice_axis]) / self.n_of_subvols
            )

            self.subvol_classifier = SubvolClassifier(
                n=self.n_of_subvols, xc=self.subvol_center
            )
            ...
            self.n_of_subvols = int(self.args.subvolumes[1])
            self.subvol_center = subvolumes.distribute(
                self.mesh, self.n_of_subvols, self.folder, view=True
            )
            ...
            if (self.grid == 1).sum() == 2:
                print(
                    "1D subvolume distribution should use 'slice' type, not 'grid'. Defaulting to 'slice'..."
                )
            ...
            self.subvol_center = (
                np.vstack(list(map(np.ravel, g))).T
            ) * self.bounds.ptp(axis=0) + self.bounds[
                0, :
            ]  # create centers
    ...
    def get_bound_facets(self, args):
        ...
        self.bound_cond = np.array(
            [args.bound_cond[-1] for _ in range(self.n_of_facets)]
        )
        ...
        if args.bound_pos[0] == "relative":
            ...
            raise Exception(
                'Please specify the type of position for BC with the keyword "absolute" or "relative".'
            )
        ...
        self.res_facets = np.arange(self.n_of_facets, dtype=int)[
            np.logical_or(self.bound_cond == "T", self.bound_cond == "F")
        ]
        self.res_bound_cond = self.bound_cond[
            np.logical_or(self.bound_cond == "T", self.bound_cond == "F")
        ]
        self.rough_facets = np.arange(self.n_of_facets, dtype=int)[
            self.bound_cond == "R"
        ]
    ...
    def check_facet_connections(self, args):
        ...
        if len(args.connect_pos) > 0:
            ...
            if args.connect_pos[0] == "relative":
                ...
                raise Exception(
                    "Wrong option in --connect_pos. Choose between 'relative' or 'absolute'."
                )
        ...
        for i in range(self.connected_facets.shape[0]):
            ...
            normal_check = np.all(
                np.absolute(normal_1 + normal_2) < 10**-self.tol_decimals
            )
            ...
            if normal_check:
                ...
                mesh_1 = Mesh(
                    vertices=self.mesh.vertices, faces=self.mesh.faces[faces_1, :]
                )
                mesh_2 = Mesh(
                    vertices=self.mesh.vertices, faces=self.mesh.faces[faces_2, :]
                )
                ...
                vertices_1_2d, b1, b2 = geo3d.transform_3d_to_2d(
                    mesh_1.vertices, normal_1, np.zeros(3)
                )
                vertices_2_2d, _, _ = geo3d.transform_3d_to_2d(
                    mesh_2.vertices, normal_1, np.zeros(3), b1=b1, b2=b2
                )

                vertices_1_2d, ring_1 = self.remove_midpoints_from_ring(
                    vertices_1_2d, ring_1
                )
                vertices_2_2d, ring_2 = self.remove_midpoints_from_ring(
                    vertices_2_2d, ring_2
                )
    ...
    def plot_mesh_bc(self):
        ...
        fig, ax = self.plot_facet_boundaries(
            self.mesh, l_color="lightgrey", number_facets=False
        )
        ...
        legend_elements = [
            Patch(facecolor="w", edgecolor="k", linestyle="-", label="Roughness"),
    ...
    def get_boundary_rings(self, mesh, fct=0):
        ...
        while np.any(active):
            loc = np.logical_and(
                np.any(bound_edges == v, axis=1), active
            ).argmax()  # where v is found on bound_edges
            e = bound_edges[loc, :]  # get the edge
            next_v = e[e != v][0]
            active[loc] = False
            if (
                next_v in ring
            ):  # if the next vertex is already registered, close the ring
                ring.append(next_v)
    ...
    def get_external_ring_index(self, v, ring_list):
        ring_bounds = [
            np.vstack((v[ring, :].min(axis=0), v[ring, :].max(axis=0)))
            for ring in ring_list
        ]

        ext_i = 0  # assume exterior ring is the first
        for i, rbound in enumerate(ring_bounds):
            if np.all(rbound[0, :] <= ring_bounds[ext_i][0, :]) and np.all(
                rbound[1, :] >= ring_bounds[ext_i][1, :]
            ):
                ext_i = i
    ...
    def plot_triangles(
        ...
        if ax is None or fig is None:
            fig, ax = plt.subplots(
                nrows=1, ncols=1, dpi=200, subplot_kw={"projection": "3d"}
            )
        ...
        for fct in facets:
            for e in mesh.facets_boundary[fct]:
                ax.plot(
                    ...
                    mesh.vertices[mesh.edges[e, :], 1],
                    mesh.vertices[mesh.edges[e, :], 2],
                    linestyle=linestyle,
                    color=l_color,
                )
            if number_facets:
    ...
    def get_subvol_connections(self):
        ...
        o = (
            self.subvol_center + np.expand_dims(self.subvol_center, 1)
        ) / 2  # interface origins/midpoints (SV, SV, 3)
        n = self.subvol_center - np.expand_dims(
            self.subvol_center, 1
        )  # interface normals/directions (SV, SV, 3)
        c_d = np.linalg.norm(n, axis=-1)  # distances (SV, SV)

        if self.subvol_type == "slice":
            sv_con = np.vstack(
                (np.arange(self.n_of_subvols - 1), np.arange(self.n_of_subvols - 1))
            ).T
            sv_con[:, 1] += 1
            self.subvol_connections = sv_con
            self.n_of_subvol_con = self.subvol_connections.shape[0]
            self.subvol_con_vectors = (
                self.subvol_center[self.subvol_connections[:, 1], :]
                - self.subvol_center[self.subvol_connections[:, 0], :]
            )
            self.save_subvol_connections()
            return

        psbl_con = np.ones(
            (self.n_of_subvols, self.n_of_subvols), dtype=bool
        )  # possible connections matrix
        ...
        order = np.argsort(
            c_d[sv_con[:, 0], sv_con[:, 1]]
        )  # setting the order by proximity

        for index, con in enumerate(
            sv_con[order, :]
        ):  # for each connection, in order of distance
            ...
            if confirmed[order[index]]:
                ...
                # check for i
                i_con = np.any(sv_con == i_sv, axis=1).nonzero()[
                    0
                ]  # index of possible connections

                if np.any(confirmed[i_con]):  # if any of these are already confirmed
                    i_con_conf = i_con[confirmed[i_con]]  # get which of them are
                    end_sv = sv_con[i_con_conf, :][
                        sv_con[i_con_conf, :] != i_sv
                    ]  # get the confirmed connected subvols
                    for k_sv in end_sv:  # for each extra sv
                        d_p = np.sum(
                            (o[i_sv, j_sv, :] - o[i_sv, k_sv, :]) * n[i_sv, k_sv, :]
                        )  # calculate the distance from origin to confirmed plane
                ...
                if not remove[order[index]]:
                    # check for j
                    j_con = np.any(sv_con == j_sv, axis=1).nonzero()[
                        0
                    ]  # index of possible connections
                    if np.any(
                        confirmed[j_con]
                    ):  # if any of these are already confirmed and was not already removed
                        j_con_conf = j_con[confirmed[j_con]]  # get which of them are
                        end_sv = sv_con[j_con_conf, :][
                            sv_con[j_con_conf, :] != j_sv
                        ]  # get the confirmed connected subvols
                        for k_sv in end_sv:  # for each extra sv
                            d_p = np.sum(
                                (o[i_sv, j_sv, :] - o[j_sv, k_sv, :]) * n[j_sv, k_sv, :]
                            )  # calculate the distance from origin to confirmed plane
                            if d_p >= 0:  # if it is not in a proper position
                                remove[order[index]] = True
    ...
    def save_subvol_connections(self):
        ...
        for i in range(self.n_of_subvols):
            ax.text(
                self.subvol_center[i, 0],
                self.subvol_center[i, 1],
                self.subvol_center[i, 2],
                "{:d}".format(i),
            )
    ...
    def plot_triangulation(
        self, mesh=None, fig=None, ax=None, l_color="k", linestyle="-", dpi=200
    ):
        if mesh is None:
            mesh = self.mesh
        fig, ax = self.mesh.plot_triangulation(
            fig=fig, ax=ax, l_color=l_color, linestyle=linestyle, dpi=dpi
        )
...
class SubvolClassifier:
    def __init__(self, n, xc=None, a=None):
        ...
        if xc is None:
            ...
            self.xc[:, self.a] = np.linspace(0, 1 - 1 / n, n) + 1 / (
                2 * n
            )  # center positions
        else:
            self.xc = xc
