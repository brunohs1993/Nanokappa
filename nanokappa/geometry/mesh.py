...
class Mesh:
    ...
    def correct_vertices_input(self, vertices):
        ...
        if vertices.shape[1] == 2:
            ...
            Exception(
                "Wrong dimensionality of passed vertices. 2D and 3D points are accepted only."
            )
    ...
    def get_edges(self):
        ...
        for f in self.faces:
            self.edges = np.vstack(
                (self.edges, np.array([[f[0], f[1]], [f[0], f[2]], [f[1], f[2]]]))
            )
        ...
        self.face_edges = -np.ones(
            self.faces.shape, dtype=int
        )  # which edges compose each face
        for i, f in enumerate(self.faces):
            e = np.array([[f[0], f[1]], [f[0], f[2]], [f[1], f[2]]])  # (3, 2)
    ...
    def check_winding(self):
        ...
        for f, face in enumerate(self.faces):  # for each origin face
            for i, h in enumerate(hits[f, :]):  # for each other face it may hit
                if h:  # if the normal hits the plane and it is not
                    bar = geo3d.cartesian_to_barycentric(
                        x[i, f, :], self.vertices[self.faces[i, :], :]
                    )  # calculate the barycentric coordinates of the collision
                    bar = np.around(bar, decimals=10)
                    if np.any(bar < 0) or np.any(
                        bar > 1
                    ):  # if it is out of the triangle
                        new_hits[f, i] = False
        ...
        for f, nh in enumerate(n_hits):  # for each number of hits
            if nh > 1:  # if the number of hits is more than one
                n_hits[f] = np.unique(
                    np.around(x[new_hits[f, :], f, :], decimals=8), axis=0
                ).shape[0]
    ...
    def get_face_adjacency(self):
        ...
        for i in range(self.n_of_edges):
            ...
            if pair.sum() == 2:  # if edge is shared by two faces, store the pair
                self.face_adjacency = np.vstack(
                    (self.face_adjacency, pair.nonzero()[0])
                )
            elif (
                pair.sum() > 2
            ):  # if it is shared by more than two triangles, store their combinations
                indices = pair.nonzero()[0]
                self.face_adjacency = np.vstack(
                    (self.face_adjacency, np.array(tuple(combinations(indices, 2))))
                )

        self.face_adjacency = np.sort(self.face_adjacency, axis=1)
        self.face_adjacency = self.face_adjacency[
            np.lexsort((self.face_adjacency[:, 1], self.face_adjacency[:, 0])), :
        ]
    ...
    def get_facet_adjacency(self):
        ...
        for fct in self.facets:
            ...
            fct_ngb = np.unique(
                self.face_adjacency[indices, :]
            )  # get adjacencent faces to the composing faces
            fct_ngb = fct_ngb[
                ~np.isin(fct_ngb, fct)
            ]  # remove those that compose the facet

            fct_ngb = np.unique(
                self.faces_to_facets(fct_ngb)
            )  # transform in facets and get the unique
    ...
    def get_faces_properties(self):
        ...
        for i, f in enumerate(self.faces):
            ...
            self.face_areas[i] = (
                np.linalg.norm(np.cross(v[1, :] - v[0, :], v[2, :] - v[0, :])) / 2
            )
            self.face_centroid[i, :] = v.mean(axis=0)

        self.area = np.sum(self.face_areas)

        self.face_basis = np.concatenate(
            (
                np.expand_dims(
        ...
        self.face_basis_matrix = np.concatenate(
            (self.face_basis, np.expand_dims(self.face_normals, axis=0)), axis=0
        )  # (3, F, 3)
        self.face_basis_matrix = np.transpose(
            self.face_basis_matrix, (1, 2, 0)
        )  # (3, F, 3)
        ...
        for f in self.faces:
            self.face_bounds = np.concatenate(
                (
                    self.face_bounds,
                    np.expand_dims(
                        np.vstack(
                            (
                                self.vertices[f, :].min(axis=0),
                                self.vertices[f, :].max(axis=0),
                            )
                        ),
                        axis=1,
                    ),
    ...
    def get_facets_properties(self, tol=None):
        ...
        for f in range(self.n_of_faces):
            ...
            if not saved:
                if f in adj_cop and not saved:
                    in_facet = np.unique(
                        adj_cop[np.any(adj_cop == f, axis=1), :]
                    )  # getting all first adjacents
                    found_all = False
                    while not found_all:
                        n_faces = in_facet.shape[0]
                        in_facet = np.unique(
                            adj_cop[np.any(np.isin(adj_cop, in_facet), axis=1), :]
                        )
                    ...
                    )  # if it doesn't have any coplanar adjacent faces, it is a facet of one face only

        self.facets = [
            np.sort(fct) for fct in self.facets
        ]  # sorting to avoid duplicates

        self.n_of_facets = len(self.facets)

        self.facets_normal = np.array(
            [self.face_normals[fct[0], :] for fct in self.facets]
        )
        self.facets_area = np.array([self.face_areas[fct].sum() for fct in self.facets])
        self.facet_centroid = np.array(
            [
                np.sum(
                    self.face_centroid[fct, :] * self.face_areas[fct].reshape(-1, 1),
                    axis=0,
                )
                / self.facets_area[i]
        ...
        self.facet_vertices = [
            self.vertices[np.unique(self.faces[self.facets[i], :]), :]
            for i in range(self.n_of_facets)
        ]

        edges_count = [
            np.unique(self.face_edges[fct, :].ravel(), return_counts=True)
            for fct in self.facets
        ]

        self.facets_edges = [
            i[0].astype(int) for i in edges_count
        ]  # edges composing the facet
        self.facets_boundary = [
            i[0][i[1] == 1].astype(int) for i in edges_count
        ]  # edges of the facet that are not shared in the facet

        self.facets_origin = np.array(
            [self.vertices[self.faces[i[0], 0], :] for i in self.facets]
        )
    ...
    def get_interfaces(self):
        ...
        self.interfacets = np.array(
            self.interfacets, dtype=int
        )  # which facets are internal
        if len(self.interfacets) > 0:
            self.interfaces = np.unique(
                np.concatenate([self.facets[i] for i in self.interfacets]).astype(int)
            )  # which faces are internal
        else:
            ...
            while inv_failed:
                ...
                for e in self.edges[edges, :]:
                    n = int(
                        np.ceil(
                            np.linalg.norm(
                                self.vertices[e[1], :] - self.vertices[e[0], :]
                            )
                            / max_edge_div
                        )
                    )
                    new = (
                        self.vertices[e[0], :]
                        + (self.vertices[e[1], :] - self.vertices[e[0], :])
                        * np.arange(1, n).reshape(-1, 1)
                        / n
                    )
                    self.simplices_points = np.vstack((self.simplices_points, new))

                self.simplices_points = np.vstack(
                    (self.simplices_points, self.vertices)
                )
                if sample_surface or sample_volume:
                    ...
                    grid = (np.vstack(list(map(np.ravel, g))).T) * self.bounds.ptp(
                        axis=0
                    ) + self.bounds[
                        0, :
                    ]  # create centers
                    ...
                    if sample_volume:
                        ...
                        self.simplices_points = np.vstack(
                            (self.simplices_points, grid[keep, :])
                        )
                    ...
                    if sample_surface:
                        ...
                        for d, ind in enumerate([[1, 2], [0, 2], [0, 1]]):
                            ...
                            ) * self.extents[ind] + self.bounds[0, ind]
                        ...
                        self.simplices_points = np.vstack(
                            (self.simplices_points, new_points)
                        )
                ...
                v1 = (
                    self.simplices_points[tri.simplices[:, 1], :]
                    - self.simplices_points[tri.simplices[:, 0], :]
                )
                v2 = (
                    self.simplices_points[tri.simplices[:, 2], :]
                    - self.simplices_points[tri.simplices[:, 0], :]
                )
                v3 = (
                    self.simplices_points[tri.simplices[:, 3], :]
                    - self.simplices_points[tri.simplices[:, 0], :]
                )
                ...
                self.simplices = np.delete(
                    tri.simplices, to_del, axis=0
                )  # keep the simplices

                self.n_of_simplices = self.simplices.shape[0]

                self.simplices_bounds = np.concatenate(
                    (
                        np.expand_dims(
                            self.simplices_points[self.simplices, :].min(axis=1), 0
                        ),
                        np.expand_dims(
                            self.simplices_points[self.simplices, :].max(axis=1), 0
                        ),
                    ),
                    axis=0,
                )  # and their bounds
                ...
                v1 = (
                    ...
                    - self.simplices_points[self.simplices[:, 0], :]
                )
                v2 = (
                ...
                v3 = (
                    self.simplices_points[self.simplices[:, 3], :]
                    - self.simplices_points[self.simplices[:, 0], :]
                )
    ...
    def plot_triangulation(
        self, fig=None, ax=None, l_color="k", linestyle="-", dpi=200
    ):
        ...
        for l in lines:
            ax.plot(
                self.simplices_points[l, 0],
                self.simplices_points[l, 1],
                self.simplices_points[l, 2],
                color=l_color,
                linestyle=linestyle,
                linewidth=1,
            )
    ...
    def contains(self, x):
        ...
        contained = np.logical_and(
            np.all(np.expand_dims(x, 1) >= self.simplices_bounds[0, :, :], axis=2),
        ...
        #                    (C, 3, 3)                   (C, 3, 1)
        bar = np.sum(
            self.contains_matrices[in_s, :, :] * np.expand_dims(b, 2), axis=1
        )  # (C, (b, c, d))
        bar = np.concatenate(
            (1 - np.sum(bar, axis=1, keepdims=True), bar), axis=1
        )  # adding a
    ...
    def get_volume_properties(self):
        ...
        self.triangulate_volume(
            max_edge_div=max_div, options="Qc Qt", sample_volume=False
        )
    ...
    def remove_vertices(self, indices, update=True):
        ...
        remove_i = (~np.isin(all_i, keep_i)).nonzero()[
            0
        ]  # is this just to convert to array? Isn't remove_i == indices?
    ...
    def remove_duplicates(self, tol=None):
        ...
        while flag:
            ...
            while not found:
                ...
                same_v = (
                    np.linalg.norm(self.vertices - self.vertices[i, :], axis=1) < tol
                )
                same_v[i] = False
        ...
        _, inverse, counts = np.unique(
            np.sort(self.faces, axis=1), axis=0, return_counts=True, return_inverse=True
        )
    ...
    def plot_triangles(
        ...
        if ax is None or fig is None:
            fig, ax = plt.subplots(
                nrows=1, ncols=1, dpi=dpi, subplot_kw={"projection": "3d"}
            )
            ax.set_box_aspect(np.ptp(self.bounds, axis=0))
    ...
    def plot_facet_boundaries(
        ...
        for fct in facets:
            for e in self.facets_boundary[fct]:
                ax.plot(
                    self.vertices[self.edges[e, :], 0],
                    self.vertices[self.edges[e, :], 1],
                    self.vertices[self.edges[e, :], 2],
                    linestyle=linestyle,
                    color=l_color,
                )
            if number_facets:
    ...
    def closest_face(self, x):
        ...
        pj = np.expand_dims(x, 1) - self.face_normals * np.expand_dims(
            np.sum(
                self.face_normals * (np.expand_dims(x, 1) - self.face_origins), axis=-1
            ),
            2,
        )  # (P, F, 3)

        valid = np.logical_and(
            np.all(pj >= (self.face_bounds[0, :, :] - self.tol), axis=2),  # (P, F)
            np.all(pj <= (self.face_bounds[1, :, :] + self.tol), axis=2),
        )  # with tolerance margin to avoid numerical errors
        ...
        valid[in_p, in_f] = np.all(
            np.logical_and(bar >= 0 - self.tol, bar <= 1 + self.tol), axis=-1
        )
    ...
    def contains_naive(self, x):
        ...
        for i in edge_closer:
            normals[i, :] = self.face_normals[self.edges_faces[e[contains][i]], :].mean(
                axis=0
            )
    ...
    def find_boundary(self, x, v):
        ...
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            ...
            t = -(
                np.sum(np.expand_dims(x, 1) * self.face_normals, axis=2) + self.face_k
            ) / np.sum(
                np.expand_dims(v, 1) * self.face_normals, axis=2
            )  # (P, F)
        ...
        in_bounds = np.logical_and(
            np.all(c >= self.face_bounds[0, in_f, :] - self.tol, axis=1),
            np.all(c <= self.face_bounds[1, in_f, :] + self.tol, axis=1),
        )  # with tolerance margin to avoid numerical errors
    ...
    def find_boundary_multiple(self, x, v):
        ...
        while np.any(active):
            xci, tci, fci = self.find_boundary(
                x[active, :], v[active, :]
            )  # get new collisions
    ...
    def sample_volume(self, n):
        if self.n_of_simplices == 0:
            raise Exception(
                "Number of simplices is zero. The mesh may be a plane and has no volume to sample from."
            )

        s = np.random.choice(
            self.n_of_simplices,
            size=n,
            p=self.simplices_volumes / self.simplices_volumes.sum(),
        )  # from which simplex to draw the points
    ...
    def sample_volume_naive(self, n):
        ...
        while points.shape[0] < n:
            new_points = (
                np.random.rand(n - points.shape[0], 3) * self.extents
                + self.bounds[0, :]
            )
    ...
    def sample_surface(self, n, faces=None, facets=None):
        ...
        f = np.random.choice(
            faces, size=n, p=self.face_areas[faces] / self.face_areas[faces].sum()
        )  # from which faces to draw the points
    ...
    def export_stl(self, name, path=None):
        ...
        for f in range(self.n_of_faces):
            content += "facet normal {:.6e} {:.6e} {:.6e}\n".format(
                self.face_normals[f, 0],
                self.face_normals[f, 1],
                self.face_normals[f, 2],
            )
            content += "    outer loop\n"
            for v in range(3):
                content += "        vertex {:.6e} {:.6e} {:.6e}\n".format(
                    self.vertices[self.faces[f, v], 0],
