...
def distribute(geo, n_r, folder, view=True):
    ...
    while not solution_found:
        ...
        while counter < tries * it:
            ...
            cvr = get_cover(
                r, n_r
            )  # get what percentage of the volume each sphere contains
        ...
        print(
            "{:4d} - Samples: {:.2e} - Max dx_r = {:.3e}".format(
                counter, n_s, np.max(dx_r)
            )
        )

    if view:  # visualise results if requested
        view_subvols(geo, folder, centers_conv, cover_conv, displacement_conv)

    return x_r


def view_subvols(geo, folder, centers_conv, cover_conv, displacement_conv):
