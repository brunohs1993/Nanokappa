...
class Visualisation(Constants):
    ...
    def set_style_dicts(self):
        ...
        if self.geometry.subvol_type == "slice":
            self.profile_plot_style = dict(
                linestyle=":", color=linecolor, marker="o", markersize=5, capsize=5
            )
        else:
            self.profile_plot_style = dict(
                linestyle="None", color=linecolor, marker="o", markersize=5, capsize=5
            )

        self.convergence_plot_style = dict(linestyle="-")

        self.grid_style = dict(ls="--", lw=1, color=gridcolor)
        ...
        self.mean_plot_style = dict(linestyle="--", color=linecolor)

        self.stdev_plot_style = dict(color="r", linestyle="--")
    ...
    def postprocess(self, verbose=True):
        with warnings.catch_warnings():
            ...
            if self.sim_time.shape[0] > 1:
                self.plot_convergence_general(
                    property_list=["e", "T", "Np", "phi", "kappa"], cmap=None
                )
    ...
    def read_convergence(self):
        ...
        self.phi_res = data[
            :, 4 + self.n_of_reservoirs : 4 + 4 * self.n_of_reservoirs
        ].astype(float)
        ...
        self.T = data[
            :,
            5
            + 4 * self.n_of_reservoirs : 5
            + 4 * self.n_of_reservoirs
            + self.n_of_subvols,
        ].astype(float)
        self.sv_en = data[
            :,
        ...
        if self.geometry.subvol_type == "slice":
            self.sv_k = data[
                ...
                5
                + 4 * self.n_of_reservoirs
                + 6 * self.n_of_subvols : 5
                + 4 * self.n_of_reservoirs
                + 7 * self.n_of_subvols,
            ].astype(float)
            self.k = data[
                :, 5 + 4 * self.n_of_reservoirs + 7 * self.n_of_subvols
            ].astype(float)
        else:
            self.con_k = data[
                :,
        ...
        if self.geometry.subvol_type == "slice":
            ...
            self.mean_con_dT = np.nanmean(
                self.T[-N:, self.geometry.subvol_connections[:, 1]]
                - self.T[-N:, self.geometry.subvol_connections[:, 0]],
                axis=0,
            )
            ...
            for i, con in enumerate(self.geometry.subvol_connections):
                phi = (
                    self.sv_phi[-N:, 3 * con[0] : 3 * (con[0] + 1)]
                    + self.sv_phi[-N:, 3 * con[1] : 3 * (con[1] + 1)]
                ) / 2
    ...
    def read_particles(self, verbose=True):
        ...
        if self.population is not None:
            ...
            if (
                ...
                and self.geometry.subvol_type == "slice"
            ):
                self.temperatures = self.temperature_interpolator(
                    self.positions[:, self.geometry.slice_axis]
                )
            else:
    ...
    def set_temperature_interpolator(self):
        ...
        if (
            ...
            self.temperature_interpolator = interp1d(
                self.geometry.subvol_center[:, self.geometry.slice_axis],
                self.mean_T,
                kind=self.temp_interp_type,
                fill_value="extrapolate",
            )
        elif self.temp_interp_type == "nearest":
            self.temperature_interpolator = NearestNDInterpolator(
                self.geometry.subvol_center, self.mean_T
            )
        elif self.temp_interp_type == "radial":
            self.temperature_interpolator = RBFInterpolator(
                self.geometry.subvol_center, self.mean_T, kernel="linear"
            )
    ...
    def plot_convergence_general(
        ...
        for prop in property_list:
            ...
            if prop in ["temperature", "T"]:
                ...
                conv_labels = [
                    "Sv {:d}".format(i)
                    for i in np.arange(self.geometry.n_of_subvols, dtype=int)
                ]
                ...
                ylabel = [
                    r"Local $\phi_{{{}}}$ [W/m²]".format(i) for i in ["x", "y", "z"]
                ]
                nrows = 3
                sharey = "all"
                sharex = "col"
                ...
                if self.geometry.subvol_type == "slice":
                    ...
                    ylabel = [
                        r"{} $\kappa$ [W/m$\cdot$K]".format(i)
                        for i in ["Local", "Total"]
                    ]
                    nrows = 2
                    sharey = "all"
                    sharex = False
                    ...
                    prof_xticklabels = [
                        "{:d}-{:d}".format(i[0], i[1])
                        for i in self.geometry.subvol_connections
                    ]
                    conv_labels = [
                        "Con {:d}-{:d}".format(i[0], i[1])
                        for i in self.geometry.subvol_connections
                    ]
            ...
            cell_length = (
                5  # the standard size is cell_length*1.5*ncols, cell_length*nrows
            )
            ncols = 2
            ...
            if cmap is not None:
                if nrows == 1:
                    ...
                    ax[0].set_prop_cycle(
                        plt.cycler("color", cmap(np.linspace(0, 1, n_lines)))
                    )
                elif nrows == 2:
                    n_lines = data.shape[1]
                    ax["left"].set_prop_cycle(
                        plt.cycler("color", cmap(np.linspace(0, 1, n_lines)))
                    )
                else:
            ...
            if nrows == 3:
                for d in range(3):
                    ...
                    if plot_mean:
                        ax[d, 0].plot(
                            conv_x,
                            data[:, np.arange(n_plot) * 3 + d].mean(axis=1),
                            **mean_dict
                        )
                    ax[d, 1].errorbar(
                        prof_x,
                        mean_prof[np.arange(n_plot) * 3 + d],
                        yerr=std_prof[np.arange(n_plot) * 3 + d],
                        **prof_dict
                    )
                ...
                ax[-1, 1].set_xticklabels(
                    prof_xticklabels, fontdict=prof_xtick_fontdict
                )
                if len(conv_labels) <= 70:
                    for a in ax[:, 0]:
                        a.legend(
                            conv_labels,
                            ncols=1 + len(conv_labels) // 10,
                ...
                rol_mean = np.convolve(data_total, np.ones(N) / N, mode="full")[
                    : -N + 1
                ]
                ...
                rol_std = np.array(
                    [
                        np.sum(
                            (data_total[max(i - N + 1, 0) : i + 1] - rol_mean[i]) ** 2
                        )
                        / (min(N, i + 1))
                ...
                ax["bottom"].legend(
                    [
                        "Instantaneous",
                        r"Rolling mean ({} datapoints)".format(N),
                        r"Rolling stdev ({} datapoints)".format(N),
                    ],
                    facecolor=self.ax_style["facecolor"],
                    edgecolor=self.ax_style["axiscolor"],
                    labelcolor=self.ax_style["textcolor"],
                ...
                text_y = (
                    min(0, np.nanmin(mean_prof) * 1.5)
                    + (
                        max(0, np.nanmax(mean_prof) * 1.5)
                        - min(0, np.nanmin(mean_prof) * 1.5)
                    )
                    * 0.75
                )
                ...
                ax["bottom"].text(
                    conv_x[-1],
                    text_y,
                    r"$\kappa$ = {:.2f}$\pm${:.2f} W/m$\cdot$K".format(
                        rol_mean[-1], rol_std[-1]
                    ),
                    ha="right",
                    bbox=props,
                )
            ...
            if type(ax) == dict:
                for key in ax.keys():
                    ...
                    ax[key].ticklabel_format(
                        axis="y", style="sci", scilimits=(0, 3), useOffset=False
                    )
            else:
                for a in ax.ravel():
            ...
            plt.suptitle(
                suptitle, fontsize="xx-large", color=self.ax_style["textcolor"]
            )  # figure title
    ...
    def flux_contribution(self):
        ...
        phi = self.phonon.normalise_to_density(
            self.hbar * dn.reshape(-1, 1) * self.omega.reshape(-1, 1) * self.velocity
        )  # eV/ps a²
        phi *= self.eVpsa2_in_Wm2  # W/m²

        dX = self.geometry.subvol_con_vectors * self.a_in_m  # m
        dT = (
            self.mean_T[self.geometry.subvol_connections[:, 1]]
            - self.mean_T[self.geometry.subvol_connections[:, 0]]
        )
        ...
        if self.geometry.subvol_type == "slice":
            mean_k = (
                self.mean_sv_k[self.geometry.subvol_connections[:, 0]]
                * self.mean_sv_Np[self.geometry.subvol_connections[:, 0]]
                + self.mean_sv_k[self.geometry.subvol_connections[:, 1]]
                * self.mean_sv_Np[self.geometry.subvol_connections[:, 1]]
            )
            mean_k /= self.mean_sv_Np[self.geometry.subvol_connections].sum(axis=1)

            std_k = (
                self.std_sv_k[self.geometry.subvol_connections[:, 0]]
                * self.mean_sv_Np[self.geometry.subvol_connections[:, 0]]
                + self.std_sv_k[self.geometry.subvol_connections[:, 1]]
                * self.mean_sv_Np[self.geometry.subvol_connections[:, 1]]
            )
        ...
        fig, ax = plt.subplots(
            nrows=2, ncols=1, figsize=(15, 20), dpi=100, sharex="all"
        )

        for c, con in enumerate(self.geometry.subvol_connections):
            if plot_con[c]:
                i = np.logical_or(
                    self.subvol_id == con[0], self.subvol_id == con[1]
                ).nonzero()[0]
                ...
                y, _, _ = ax[0].hist(
                    ...
                    bins=bins,
                    weights=k,
                    label="Con. {:d}-{:d}".format(con[0], con[1]),
                    histtype="step",
                )

                ax[1].plot(
                    centers, np.cumsum(y), label="Con. {:d}-{:d}".format(con[0], con[1])
                )
        ...
        ax[0].set_ylabel(
            r"Thermal conductivity in band $k(\omega)$ [W/mK]", fontsize="x-large"
        )
        ...
        ax[1].set_ylabel(
            r"Cumulated Thermal conductivity in band $k(\omega)$ [W/mK]",
            fontsize="x-large",
        )
        ...
        plt.suptitle(
            "Contribution of each frequency band to thermal conductivity. {:d} bands.".format(
                centers.shape[0]
            ),
            fontsize="xx-large",
        )
    ...
    def plot_kappa_path(self):
        ...
        for i, con in enumerate(self.geometry.subvol_connections):
            if k[i] != np.nan:
                ax.plot(
                    self.geometry.subvol_center[con, 0],
